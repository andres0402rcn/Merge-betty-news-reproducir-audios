...
@Component({
  selector: 'app-media-details-betty',
  templateUrl: './media-details-betty.component.html',
  styleUrls: ['./media-details-betty.component.css']
})
export class MediaDetailsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    if (this.storageService.getToken()) {
      ...
      this.interval = setInterval(() => this.voiceover ? this.filterAudioFiles() : this.loadData(), 180000);
    }
  }
  ...
  loadData(): void {
    ...
    this.mediaDetailsSubscription = this.mediaDetailService.getListMediaDetail().subscribe({
      next: (data) => {
  ...
  // Sección confirmación de acciones
  ...
  downloadFile(aiFileId: number, aiFilePath: string): void {
    this.aiFileService.downloadFile(aiFileId).subscribe({
      next: (blob) => {
  ...
  // Sección generación de proxy de video
  ...
  public filterVoiceoverFiles: DropDownFilterSettings = {
    ...
    this.isLoading = true;
    document.body.style.cursor = 'wait';
      this.mediaDetailAudioSubscription = this.mediaDetailService.getListMediaDetailAudios().subscribe({
        next: (data) => {
          this.mediaDetails = JSON.parse(data);
          this.gridData = process(this.mediaDetails, this.state);
          this.loading = false;
